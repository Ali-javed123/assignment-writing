<?php include('header.php');?>
    <!--body content wrap start-->
    <div class="main">

        <!--hero section start-->
        <section class="hero-section ptb-100 gradient-overlay"
            style="background: url('img/macbook-pro-air-acd.jpg')no-repeat center center / cover">
            <div class="container">
                <div class="row align-items-center justify-content-center">
                    <div class="col-md-9 col-lg-7">
                        <div class="page-header-content text-white text-center pt-sm-5 pt-md-5 pt-lg-0">
                            <h2 class="text-white mb-0">Free Samples of Design
                            </h2>
                            <p><i class="fa fa-leaf"></i><b>Free Tumitin Report</b></p>
                            <p><i class="fa fa-flag"></i><b>7,00000+ Total Satisfied Student</b></p>
                            <p><i class="fa fa-thumbs-up"></i><b>150+ Executive for Support Service</b></p>


                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!--hero section end-->
        <div class="container ">
          <h2 class="text-center">Get the Best Design to Improve Your Grade


        </h2>
        <i class="fas fa-arrow-circle-down"></i>
          <div class="row row-4 justify-content-center">
            <div class="col-md-6 text-c card">
                <!-- sub row start -->
                <h3 class="text-center"><a href="design-2.php">Game Design:</a></h3>
                <div class="row">
   
                    <div class="col-md-8">
                        <h5>Numbers of Pages:</h5>
                    </div>
                    <div class="col-md-4"> 
                        <h5 class="text-right">10</h5>
                    </div>
                    <div class="col-md-7">
                      <h5>Writters Qualification:</h5> 
                  </div>
                  <div class="col-md-5"> 
                      <h5 class="text-right">Graduation</h5>
                  </div>
                  <div class="col-md-7">
                      <h5>Academic Level:</h5>
                  </div>
                  <div class="col-md-5"> 
                      <h5 class="text-right">Post Graduation</h5>
                  </div>
                  <div class="col-md-4">
                    <h5>Tags:</h5>
                </div>
                <div class="col-md-8"> 
                    <h5 class="text-right">Top University In UK</h5>
                </div>
                </div>
                
            </div>
            
          </div>
            </div>
        </div>
        <?php include('footer.php');?>
       